...
namespace earth
{
  ...
  public:
    ...
    {
      // precalculations
      double lambda = earth::WGS84::geodeticToGeocentric(gamma);
      double r = earth::WGS84::geocentricRadius(lambda);
      ...
      double gmR2  = earth::WGS84::gm/R2;
      double re2R2 = (earth::WGS84::majorRadius*earth::WGS84::majorRadius)/R2;

      double gR = -gmR2*(1.0+(9.0/2.0*sqrt(5.0)*earth::WGS84::c20)*re2R2*(Z*Z/R2-1.0/3.0));
      double gT = (3.0*sqrt(5.0)*earth::WGS84::c20)*gmR2*re2R2*(XY*Z/R2);

      // gravity viewed in ECEF frame
      double gZ  = gR*slam+gT*clam;
      double gXY = gR*clam-gT*slam+earth::WGS84::rotationRate*earth::WGS84::rotationRate*XY;
      ...
      double& gE, double& gD)
    {
      ...
      double gam = earth::WGS84::geocentricToGeodetic(lam);
      double h = R-earth::WGS84::geocentricRadius(lam);
    ...
    {
      double a = earth::WGS84::majorRadius;
      double finv = earth::WGS84::inverseFlattening;
      double b = a-a/finv;
      double a2 = a*a;
      double b2 = b*b;
    ...
    {
      earth::WGS84::llaToECEF(lla[0], lla[1], lla[2], x, y, z);
      return;
    }
    static inline void llaToECEF(const double& lon, const double& lat, const double& alt, double (&xyz)[3])
    {
      earth::WGS84::llaToECEF(lon, lat, alt, xyz[0], xyz[1], xyz[2]);
      return;
    }
    static inline void llaToECEF(const double (&lla)[3], double (&xyz)[3])
    {
      earth::WGS84::llaToECEF(lla[0], lla[1], lla[2], xyz[0], xyz[1], xyz[2]);
      return;
    }
    ...
    {
      earth::WGS84::ecefToLLA(xyz[0], xyz[1], xyz[2], lon, lat, alt);
      return;
    }
    static inline void ecefToLLA(const double& x, const double& y, const double& z, double (&lla)[3])
    {
      earth::WGS84::ecefToLLA(x, y, z, lla[0], lla[1], lla[2]);
      return;
    }
    static inline void ecefToLLA(const double (&xyz)[3], double (&lla)[3])
    {
      earth::WGS84::ecefToLLA(xyz[0], xyz[1], xyz[2], lla[0], lla[1], lla[2]);
      return;
    }
    ...
    static double geocentricToGeodetic(const double& lambda)
    {
      double A;
      double gamma;
      if((lambda<-math::PI/2.0)|(lambda>math::PI/2.0))
      {
        gamma = NAN;
      }
      else
      {
        A = earth::WGS84::majorRadius/earth::WGS84::minorRadius;
    ...
    {
      double lambda;
      if((gamma<-math::PI/2.0)|(gamma>math::PI/2.0))
      {
        lambda = NAN;
      }
      else
      {
        lambda = atan2(pow(earth::WGS84::minorRadius/earth::WGS84::majorRadius, 2.0)*sin(gamma), cos(gamma));
      }
      return (lambda);
    }
    ...
    {
      double A = earth::WGS84::majorRadius*sin(lambda);
      double B = earth::WGS84::minorRadius*cos(lambda);
      double radius = (earth::WGS84::majorRadius*earth::WGS84::minorRadius)/sqrt(A*A+B*B);
      return (radius);
    }
     ...
     * @param[out] radius Radius in meters
     */
    static double radiusOfCurvature(const double& gamma)
    {
      double ab = earth::WGS84::majorRadius*earth::WGS84::minorRadius;
      double acg = earth::WGS84::majorRadius*cos(gamma);
      double bsg = earth::WGS84::minorRadius*sin(gamma);
      double radius = ab*ab*pow(acg*acg+bsg*bsg, -1.5);
      return (radius);
    }
  };
  ...
  const double WGS84::minorRadius = earth::WGS84::majorRadius-earth::WGS84::majorRadius/earth::WGS84::inverseFlattening; // meters (Implementation Manual)
  ...
  // const double WGS84::equatorialGravity = -9.7803253359-(earth::WGS84::rotationRate*earth::WGS84::rotationRate*1000000)*(earth::WGS84::majorRadius/1000000); // meters/sec^2 (combined sources)
  // const double WGS84::equatorialGravity = -9.78049-(earth::WGS84::rotationRate*earth::WGS84::rotationRate*1000000)*(earth::WGS84::majorRadius/1000000); // meters/sec^2 (Draper)
