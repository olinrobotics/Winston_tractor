classdef TCP < handle
  ...
  methods (Access = public)
    function this = TCP(varargin)
      role = varargin{1};
      switch(role)
        case 'server'
          serverPort = varargin{2};
          timeout = varargin{3};
          this.stream = jtcp('accept', serverPort, 'timeout', timeout, 'serialize', false);
        case 'client'
          serverAddress = varargin{2};
          serverPort = varargin{3};
          timeout = varargin{4};
          this.helper = fileparts(mfilename('fullpath'));
          this.stream = jtcp('request', serverAddress, serverPort, 'timeout', timeout, 'serialize', false);
          this.buffer = zeros(0, 1, 'int8');
        otherwise
          error('TCP: Role must be ''server'' or ''client''.');
      end
    end
    
    function send(this, byteArray)
      assert(isa(byteArray, 'int8'));
      assert(size(byteArray, 2)==1);
      nArray = size(byteArray, 1);
      if(nArray>0)         
        prefix = typecast(uint32(nArray), 'int8');
        jtcp('write', this.stream, prefix);
        jtcp('write', this.stream, byteArray);
      end
    end
    
    function byteArray = receive(this)
      bytes = jtcp('read', this.stream, 'helperClassPath', this.helper);
      this.buffer = [this.buffer; bytes];
      nBuffer = numel(this.buffer);
      if(nBuffer>=4)
        nArray = typecast(this.buffer(1:4), 'uint32');
        if(nBuffer>=(4+nArray))
          byteArray = this.buffer(5:(4+nArray));
          this.buffer = this.buffer((5+nArray):end);
        else
          byteArray = zeros(0, 1, 'int8');
        end
      else
        byteArray = zeros(0, 1, 'int8');
      end
...
if jTcpObj.serialize
    ...
    if numBytesToRead > numBytesAvailable
        ...
        if doUseHelperClass
            ...
            % call for each byte.
            mssg = zeros(numBytesToRead, 1, 'int8');
