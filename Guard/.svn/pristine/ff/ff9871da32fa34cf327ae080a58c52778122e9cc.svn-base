...
static const double sensorForwardM = 1.953;
...
private:
  ...
  void processLVO(const std::string& inbox)
  {
    ...
    // transform measurement from sensor frame to body frame
    if(!this->gotMeas)
    {
      this->rLatM = earth::WGS84::radiusOfCurvature(y.latR);
      this->rLonM = cos(earth::WGS84::geodeticToGeocentric(y.latR))*earth::WGS84::geodeticRadius(y.latR);
      this->gotMeas = true;
    }
    y.latR = y.latR-cos(y.yawR)*sensorForwardM/this->rLatM;
    y.lonR = y.lonR-sin(y.yawR)*sensorForwardM/this->rLonM;
