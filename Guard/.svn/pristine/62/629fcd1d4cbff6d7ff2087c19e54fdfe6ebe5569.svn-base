classdef WGS84
  properties (Constant = true, GetAccess = public)
    ...
    minorRadius = earth.WGS84.majorRadius-earth.WGS84.majorRadius/earth.WGS84.inverseFlattening; % meters (Implementation Manual)
  end
  
  methods (Access = public, Static = true)
    function [gN, gE, gD] = gravityNED(N, E, D, gamma)
      % precalculations
      lambda = earth.WGS84.geodeticToGeocentric(gamma);
      r = earth.WGS84.geocentricRadius(lambda);
      sgam = sin(gamma);
      cgam = cos(gamma);
      slam = sin(lambda);
      clam = cos(lambda);
      ...
      gmR2  = earth.WGS84.gm./R2;
      re2R2 = (earth.WGS84.majorRadius*earth.WGS84.majorRadius)./R2;

      gR = -gmR2.*(1.0+(9.0/2.0*sqrt(5.0)*earth.WGS84.c20)*re2R2.*(Z.*Z./R2-1.0/3.0));
      gT = (3.0*sqrt(5.0)*earth.WGS84.c20)*gmR2.*re2R2.*(XY.*Z./R2);

      % gravity viewed in ECEF frame
      gZ  = gR*slam+gT*clam;
      gXY = gR*clam-gT*slam+earth.WGS84.rotationRate*earth.WGS84.rotationRate*XY;
    ...
    function [gN, gE, gD] = gravityNEDNearEarth(N, E, D, gamma)
      ...
      lam = atan2(Z, XY);
      gam = earth.WGS84.geocentricToGeodetic(lam);
      h = R-earth.WGS84.geocentricRadius(lam);
    ...
    function [x, y, z] = llaToECEF(lon, lat, alt)
      ...
      a = earth.WGS84.majorRadius;
      finv = earth.WGS84.inverseFlattening;
      b = a-a/finv;
      a2 = a.*a;
      b2 = b.*b;
      e = sqrt((a2-b2)./a2);
    ...
    function [lon, lat, alt] = ecefToLLA(x, y, z)
      ...
      c = (e2*e2*F.*r2)./(G.*G.*G);
      s = power(1.0+c+sqrt(max(c.*c+2.0*c, 0.0)), 1.0/3.0);
    ...
    function lambda = geodeticToGeocentric(gamma)
      if(isnumeric(gamma))
        if(any((gamma<-pi/2)||(gamma>pi/2)))
          ...
          lambda = atan2((earth.WGS84.minorRadius/earth.WGS84.majorRadius)^2*sin(gamma), cos(gamma));
        end
      else
        syms re rp
    ...
    function radius = geocentricRadius(lambda)
      A = earth.WGS84.majorRadius*sin(lambda);
      B = earth.WGS84.minorRadius*cos(lambda);
      radius = (earth.WGS84.majorRadius*earth.WGS84.minorRadius)./sqrt(A.*A+B.*B);
    end
    ...
    function radius = radiusOfCurvature(gamma)
      a = earth.WGS84.majorRadius;
      b = earth.WGS84.minorRadius;
      ab = a*b;
      acg = a*cos(gamma);
      bsg = b*sin(gamma);
    ...
    function [OmegaNED, XddNED] = rotationEffectNED(X_rel, Xdav_rel, gamma)
      ...
      % constants
      re = earth.WGS84.minorRadius;
      rp = earth.WGS84.majorRadius;
      omega = earth.WGS84.rotationRate;
      lambda = earth.WGS84.geodeticToGeocentric(gamma);
